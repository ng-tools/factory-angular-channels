    ...
    inject(
      merge(
        ...
        vfs.src(paths.styles, {cwd: paths.cwd, base: paths.cwd})
          .pipe(require('./../styles/dist').bind(_.defaults({type: 'component', dest: path.join(docs.dest, 'dist'), name: pkg.name}, this))()),
    ...
    inject(
      merge(
        vfs.src(paths.docsScripts, {cwd: paths.cwd, base: paths.cwd})
          .pipe(require('./../scripts/dist').bind(_.defaults({type: 'component', dest: path.join(docs.dest, 'docs'), name: pkg.name + '.docs'}, this))()),
        vfs.src(paths.docsTemplates, {cwd: paths.cwd, base: paths.cwd})
          .pipe(require('./../views/dist').bind(_.defaults({type: 'component', dest: path.join(docs.dest, 'docs'), name: pkg.name + '.docs'}, this))())
      ).pipe(gulpif('**/*.js', angularFilesort())),
