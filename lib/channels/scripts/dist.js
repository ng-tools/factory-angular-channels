  ...
  return function ChannelFactory(src, options) {
    ...
    return combine(
      ...
      gulpif(/^lib/.test(config.type), combine(
        concat.scripts((config.name || config.pkg.name) + '.js', {cwd: src.cwd, base: src.cwd, passthrough: config.passthrough}),
      ...
      gulp.dest(src.dest),
      gulpif(config.usemin !== false, uglify({output: {indent_level: 2/*, quote_style: 1*/}})),
      concat.header(config.banner),
